  ...
  return (
    <div className="w-full  border border-(--input-border) rounded-xl shadow-sm  overflow-hidden">
      ...
      <button
        ...
        className={`w-full flex items-center ${expandIconDirection ? "justify-between p-4" : "justify-center p-3"}  gap-1 text-(--forground) bg-(--surface) text-sm font-medium transition-colors`}
      >
        {isFaqData(data)
          ? data.question
          : isCategoryData(data)
          ...
          >
            {isCategoryData(data) && data.recentTransactions?.length ? (
              <>
                ...
                {/* Footer Action */}
                <div className="p-4 ">
                  <p className="w-full text-center py-2.5 bg-(--surface) text-(--forground) rounded-lg text-sm font-bold transition-colors">
